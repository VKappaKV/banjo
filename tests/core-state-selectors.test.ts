...
import type { AccountHD, Address, Arc55App, WalletState } from "../src/lib/core";
...
const hdRootAddress =
  "HDROOT00000000000000000000000000000000000000000000000000";
const hdChildAddress =
  "HDCHILD0000000000000000000000000000000000000000000000000";
const appAddress = "APP000000000000000000000000000000000000000000000000000000";
const ledgerAddress =
  "LEDGER00000000000000000000000000000000000000000000000000";
const falconAddress =
  "FALCON00000000000000000000000000000000000000000000000000";
...
describe("wallet state selectors", () => {
  ...
  it("includes rekeyed subaccounts for signable authorizing accounts", () => {
    const hotAddress_asAddress = hotAddress as Address;
    const selected = selectAccountInfo(
      state({
        accounts: [{ addr: hotAddress }],
        hotKeyAddresses: [hotAddress],
        accountInfo: [
          accountInfo(rekeyAddress, { authAddr: hotAddress_asAddress }),
        ],
      }),
    );
  ...
  it("includes HD sibling accounts", () => {
    const selected = selectAccountInfo(
      state({
        ...
        accountInfo: [
          accountInfo(hdChildAddress, { sibling: hdRootAddress, addrIdx: 1 }),
        ],
      }),
    );
  ...
  it("filters sendable and signable accounts separately", () => {
    ...
    expect(
      selectSendAccounts(selectedState).map((account) => account.addr),
    ).toEqual([hotAddress, appAddress]);
    expect(
      selectSigningAccounts(selectedState).map((account) => account.addr),
    ).toEqual([hotAddress]);
  });
  ...
  it("marks Ledger slot and Falcon accounts as signable", () => {
    const selectedState = state({
      accounts: [
        ...
        {
          addr: falconAddress,
          falcon: { counter: 1, publicKey: "public-key" },
        },
      ],
    });
    ...
    expect(
      selectSigningAccounts(selectedState).map((account) => account.addr),
    ).toEqual([ledgerAddress, falconAddress]);
  });

  it("selects multisig signer by voting power", () => {
    const signer = selectMsigSigner(
      {
        addrs: [watchAddress, hotAddress, hotAddress],
        groups: [],
      } as unknown as Arc55App,
      [
        {
          addr: watchAddress,
          title: "watch",
  ...
  it("builds custom network overrides by genesis ID", () => {
    ...
    expect(
      networks.find((network) => network.name === "MainNet")?.algod.url,
    ).toBe("https://override");
  });

  it("injects LocalNet sandbox router without mutating static data", () => {
    const before = builtInNetworks.find(
      (network) => network.name === "LocalNet",
    )?.inboxRouter;
    const selected = selectNetwork(
      state({ networkName: "LocalNet", sandboxRouter: 123 }),
      builtInNetworks,
    );
    const after = builtInNetworks.find(
      (network) => network.name === "LocalNet",
    )?.inboxRouter;
  ...
  it("produces native Algo asset metadata", () => {
