...
<div class="grid gap-6">
  <div>
    <h2 class="text-2xl font-mono tracking-tight">Add Network</h2>
    <p class="text-muted-foreground text-sm">
      Review the custom network requested by the dApp.
    </p>
  </div>
  ...
  {:else if request.done}
    <Card.Root
      ><Card.Header
        ><Card.Title>Network response sent</Card.Title><Card.Description
  ...
  {:else if request.request?.action !== "network"}
    <Alert.Root variant="destructive"
      ><Alert.Description>No add-network request is pending.</Alert.Description
      ></Alert.Root
    >
  {:else}
    <Card.Root>
      <Card.Header>
        ...
        <Card.Description
          >{network?.genesisID ??
            "The requested network could not be validated."}</Card.Description
        >
      </Card.Header>
      <Card.Content class="grid gap-4">
        {#if network}
          <div class="grid grid-cols-[auto_1fr] gap-x-4 gap-y-1 text-sm">
            <span class="text-muted-foreground">Algod</span><span
              class="break-all">{network.algod.url}:{network.algod.port}</span
            >
            {#if network.indexer}<span class="text-muted-foreground"
                >Indexer</span
              ><span class="break-all"
                >{network.indexer.url}:{network.indexer.port}</span
              >{/if}
            {#if network.explorer}<span class="text-muted-foreground"
                >Explorer</span
              ><span class="break-all">{network.explorer}</span>{/if}
            {#if network.genesisHash}<span class="text-muted-foreground"
                >Genesis hash</span
              ><span class="break-all">{network.genesisHash}</span>{/if}
          </div>
        {/if}
