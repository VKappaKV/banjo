...
<div class="grid gap-4">
  <div class="flex items-center gap-3">
    ...
    <h2 class="text-xl font-mono tracking-tight">ARC-55 Multisig Import</h2>
  </div>
  ...
  {#if step === "form"}
    <Card.Root>
      <Card.Header>
        ...
        <Card.Description
          >Enter the app ID of an ARC-55 on-chain multisig app.</Card.Description
        >
      </Card.Header>
      ...
      <Card.Footer>
        <Button onclick={handleLoad} disabled={!appIdInput.trim()}
          >Load App</Button
        >
      </Card.Footer>
    </Card.Root>
  ...
  {:else if step === "confirm"}
    <Card.Root>
      <Card.Header>
        ...
        <Card.Description>
          Loaded ARC-55 app with {loadedApp?.addrs.length ?? 0} member(s), threshold
          {String(loadedApp?.arc55_threshold ?? "?")}.
        </Card.Description>
      </Card.Header>
      <Card.Content class="grid gap-2">
        {#if validationError}
          ...
          <p class="text-sm text-muted-foreground">
            You can still add this account, but you may not be able to sign for
            it.
          </p>
        {/if}
  ...
  {:else if step === "done"}
    <Card.Root>
      <Card.Header>
        ...
        <Card.Description
          >The ARC-55 multisig account has been added to your wallet.</Card.Description
        >
      </Card.Header>
      <Card.Footer class="flex gap-2">
