<script lang="ts">
  ...
  function computePreview() {
    ...
    if (threshold < 1 || threshold > addrs.length) {
      error = `Threshold must be between 1 and ${addrs.length}.`;
      return;
    }
    try {
  ...
  async function handleDeploy() {
    ...
    try {
      ...
      const account = accountFromArc55App(
        loadedApp as never,
...
<div class="grid gap-4">
  <div class="flex items-center gap-3">
    ...
    <h2 class="text-xl font-mono tracking-tight">ARC-55 Multisig Create</h2>
  </div>
  ...
  {#if step === "form"}
    <Card.Root>
      <Card.Header>
        ...
        <Card.Description
          >Set the threshold and member addresses for the on-chain multisig app.</Card.Description
        >
      </Card.Header>
      <Card.Content class="grid gap-3">
        ...
        <div class="grid gap-1">
          <label class="text-sm font-medium" for="addresses"
            >Member addresses (one per line)</label
          >
          <textarea
            id="addresses"
        ...
        {#if multisigAddr}
          <Alert.Root>
            ...
            <Alert.Description class="break-all font-mono text-xs"
              >{multisigAddr}</Alert.Description
            >
          </Alert.Root>
        {/if}
      ...
      <Card.Footer class="flex flex-wrap gap-2">
        <Button variant="outline" onclick={computePreview}
  ...
  {:else if step === "deploying"}
    <Card.Root>
      <Card.Header>
        ...
        <Card.Description
          >Creating the ARC-55 app on-chain. Please wait.</Card.Description
        >
      </Card.Header>
      <Card.Content>
        {#if error}
