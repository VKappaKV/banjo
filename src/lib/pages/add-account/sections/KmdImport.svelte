<script lang="ts">
	...
	let error = $state("");
	let candidates = $state<Awaited<ReturnType<typeof loadKmdImportCandidates>> | null>(null);
...
<div class="grid gap-4">
	<div class="flex items-center gap-3">
		...
		<h2 class="text-xl font-semibold tracking-tight">KMD Import (LocalNet)</h2>
	</div>
	...
	{#if step === "loading"}
		<Card.Root>
			<Card.Header>
				...
				<Card.Description>Connecting to the LocalNet KMD wallet.</Card.Description>
			</Card.Header>
			<Card.Content>
				{#if error}
	...
	{:else if step === "select"}
		<Card.Root>
			<Card.Header>
				...
				<Card.Description>These accounts will be imported as hot accounts.</Card.Description>
			</Card.Header>
				...
				{#each candidates?.privateAccounts ?? [] as account (account.addr as unknown as string)}
					<label class="flex cursor-pointer items-center gap-3 rounded border p-3 hover:bg-accent/50">
						<input
							...
							onchange={() => toggleCandidate(account.addr as unknown as string)}
						/>
						<div class="min-w-0">
	...
	{:else if step === "done"}
		<Card.Root>
			<Card.Header>
				...
				<Card.Description>{addedCount} KMD account(s) imported as hot accounts.</Card.Description>
			</Card.Header>
			<Card.Footer class="flex gap-2">
