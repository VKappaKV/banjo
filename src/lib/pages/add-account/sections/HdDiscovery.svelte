<script lang="ts">
	...
	import { decryptStoredSeed, deriveHdAccounts, defaultHdDiscoveryCount } from "$core/keys";
	...
	type Step = "select-seed" | "password" | "discovering" | "select-accounts" | "done";
	...
	async function handlePassword() {
		...
		if (!seedData) { error = "No encrypted seed found."; return; }
	...
	async function handleAdd() {
		...
		error = "";
		try {
			const selected = candidates.filter((c) => selectedAddresses.has(c.address));
			if (selected.length === 0) { error = "Select at least one account."; return; }
...
<div class="grid gap-4">
	<div class="flex items-center gap-3">
		...
		<h2 class="text-xl font-semibold tracking-tight">HD Account Discovery</h2>
	</div>
	...
	{#if step === "select-seed"}
		<Card.Root>
			<Card.Header>
				...
				<Card.Description>Choose a seed to discover HD accounts from.</Card.Description>
			</Card.Header>
			<Card.Content class="grid gap-2">
				{#if seeds.length === 0}
					<p class="text-sm text-muted-foreground">No seeds found. Import or create a BIP39 seed first.</p>
				{/if}
				{#each seeds as seed (seed.id)}
					<Card.Root class="cursor-pointer hover:bg-accent/50" onclick={() => proceedToPassword(seed.id)}>
						<Card.Header>
	...
	{:else if step === "password"}
		<Card.Root>
			<Card.Header>
				...
				<Card.Description>Unlock the seed to discover HD accounts.</Card.Description>
			</Card.Header>
			...
			<Card.Footer>
				<Button onclick={handlePassword} disabled={!passphrase}>Unlock & Discover</Button>
			</Card.Footer>
		</Card.Root>
	...
	{:else if step === "discovering"}
		<Card.Root>
			<Card.Header>
				...
				<Card.Description>Scanning the blockchain for HD accounts.</Card.Description>
			</Card.Header>
		</Card.Root>
		...
		<Card.Root>
			...
			<Card.Content class="grid gap-2">
				...
				{#each candidates as candidate (candidate.address)}
					<label class="flex cursor-pointer items-center gap-3 rounded border p-3 hover:bg-accent/50">
						<input
							type="checkbox"
			...
			<Card.Footer class="flex flex-wrap gap-2">
				...
				<Button variant="outline" onclick={() => discover(Math.max(0, startIndex - defaultHdDiscoveryCount))}>
					Previous
				</Button>
				<Button variant="outline" onclick={() => discover(startIndex + defaultHdDiscoveryCount)}>
					Next
				</Button>
			</Card.Footer>
		</Card.Root>
	...
	{:else if step === "done"}
		<Card.Root>
			<Card.Header>
				...
				<Card.Description>{addedCount} HD account(s) added to the wallet.</Card.Description>
			</Card.Header>
