<script lang="ts">
	...
	async function handleDerive() {
		...
		if (!seedData) { error = "No seed found."; return; }
...
<div class="grid gap-4">
	<div class="flex items-center gap-3">
		...
		<h2 class="text-xl font-semibold tracking-tight">Falcon Account</h2>
	</div>
	...
	{#if step === "select-seed"}
		<Card.Root>
			<Card.Header>
				...
				<Card.Description>Choose a BIP39 seed for Falcon key derivation. Derivation may take a moment.</Card.Description>
			</Card.Header>
			<Card.Content class="grid gap-2">
				{#if seeds.length === 0}
					<p class="text-sm text-muted-foreground">No BIP39 seeds found. Import a seed first.</p>
				{/if}
	...
	{:else if step === "password"}
		<Card.Root>
			<Card.Header>
				...
				<Card.Description>Unlock the seed for Falcon derivation.</Card.Description>
			</Card.Header>
			...
			<Card.Footer>
				<Button onclick={handleDerive} disabled={!passphrase}>Derive Falcon Account</Button>
			</Card.Footer>
		</Card.Root>
	...
	{:else if step === "deriving"}
		<Card.Root>
			<Card.Header>
				...
				<Card.Description>This may take a few moments as the key is generated and the blockchain is queried.</Card.Description>
			</Card.Header>
			<Card.Content>
				{#if error}
					<Alert.Root variant="destructive">
