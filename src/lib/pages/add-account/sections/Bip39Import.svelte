...
<div class="grid gap-4">
	<div class="flex items-center gap-3">
		...
		<h2 class="text-xl font-semibold tracking-tight">BIP39 Seed Import</h2>
	</div>

	<Card.Root>
		<Card.Header>
			<Card.Title>Import a BIP39 seed phrase</Card.Title>
			<Card.Description>Enter your 24-word BIP39 seed and a password to encrypt it locally.</Card.Description>
		</Card.Header>
		<Card.Content class="grid gap-3">
			...
			{#if seedId !== null}
				<Alert.Root>
					<Alert.Description>Seed imported with ID {seedId}. You can now discover HD accounts.</Alert.Description>
				</Alert.Root>
			{/if}
		</Card.Content>
		<Card.Footer class="flex gap-2">
			<Button onclick={handleImport} disabled={!mnemonic.trim()}>Import Seed</Button>
			{#if seedId !== null}
