<script lang="ts">
	...
	function verifyChallenge() {
		challengeError = "";
		if (!preview) return;
		if (challengeAnswer.trim().toLowerCase() !== preview.mnemonicWords[challengeIndex]!.toLowerCase()) {
...
<div class="grid gap-4">
	<div class="flex items-center gap-3">
		...
		<h2 class="text-xl font-semibold tracking-tight">New Hot Account</h2>
	</div>
	...
	{#if step === "warning"}
		<Card.Root>
			<Card.Header>
				...
				<Card.Description>
					You are about to generate a new random Algorand account key. This key will be stored in your browser's encrypted storage. Write down the mnemonic and keep it safe — it is the only way to recover this account.
				</Card.Description>
			</Card.Header>
			<Card.Footer>
	...
	{:else if step === "mnemonic"}
		<Card.Root>
			<Card.Header>
				...
				<Card.Description>Write these 25 words down in order and store them securely.</Card.Description>
			</Card.Header>
			<Card.Content class="grid gap-3">
				...
				<div class="flex items-center gap-2">
					<Button variant="ghost" size="sm" onclick={() => (revealWords = !revealWords)}>
						{revealWords ? "Hide" : "Show"} mnemonic
					</Button>
				</div>
			</Card.Content>
			<Card.Footer>
				<Button onclick={proceedToChallenge}>I've saved my mnemonic — Continue</Button>
			</Card.Footer>
		</Card.Root>
	...
	{:else if step === "challenge"}
		<Card.Root>
			<Card.Header>
				...
				<Card.Description>Type word #{challengeIndex + 1} from your mnemonic to confirm you've saved it.</Card.Description>
			</Card.Header>
			<Card.Content class="grid gap-3">
				<input
			...
			<Card.Footer>
				<Button onclick={verifyChallenge} disabled={!challengeAnswer.trim()}>Confirm</Button>
			</Card.Footer>
		</Card.Root>
	...
	{:else if step === "done"}
		<Card.Root>
			<Card.Header>
				...
				<Card.Description>Your new hot account is ready: {savedAddr.slice(0, 8)}...</Card.Description>
			</Card.Header>
			<Card.Content>
				{#if saveError}
					<Alert.Root variant="destructive">
			...
			<Card.Footer class="flex gap-2">
				...
				<Button variant="outline" onclick={() => push(`/account-detail/${savedAddr}`)}>View Details</Button>
			</Card.Footer>
		</Card.Root>
	{/if}
</div>
