<script lang="ts">
	...
	import { discoverLedgerAccounts, defaultLedgerDiscoveryCount } from "$core/keys";
...
<div class="grid gap-4">
	<div class="flex items-center gap-3">
		...
		<h2 class="text-xl font-semibold tracking-tight">Ledger Account Discovery</h2>
	</div>
	...
	{#if step === "discovering"}
		<Card.Root>
			<Card.Header>
				...
				<Card.Description>
					Connect your Ledger device and open the Algorand app. Click below to start discovery.
				</Card.Description>
			</Card.Header>
			<Card.Content>
				{#if error}
	...
	{:else if step === "select-accounts"}
		<Card.Root>
			...
			<Card.Footer class="flex flex-wrap gap-2">
				...
				<Button variant="outline" onclick={() => discover(Math.max(0, startIndex - defaultLedgerDiscoveryCount))}>
					Previous
				</Button>
				<Button variant="outline" onclick={() => discover(startIndex + defaultLedgerDiscoveryCount)}>
					Next
				</Button>
			</Card.Footer>
		</Card.Root>
	...
	{:else if step === "done"}
		<Card.Root>
			<Card.Header>
				...
				<Card.Description>{addedCount} Ledger account(s) added to the wallet.</Card.Description>
			</Card.Header>
