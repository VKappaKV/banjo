<script lang="ts">
  ...
  import {
    setMultisigPageState,
    getMultisigPageState,
  } from "./multisig-page-state.svelte";
...
<div class="grid gap-6">
  ...
  >
    <div class="flex items-center gap-3">
      <h2 class="text-2xl font-mono tracking-tight">{title}</h2>
    </div>
  </div>
  ...
  {#if !params.appId}
    <Alert.Root>
      ...
      <Alert.Description>
        Select a multisig account from the accounts page and view its details to
        access ARC-55 management.
      </Alert.Description>
    </Alert.Root>
