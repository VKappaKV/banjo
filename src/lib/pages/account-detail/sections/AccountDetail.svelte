<script lang="ts">
  ...
  } from "$lib/app/queries/wallet-queries.svelte";
  ...
  let account = $derived(
    accountQuery.data ?? app.accounts.find((a) => a.addr === addr) ?? null,
  );
  let title = $derived(
    account?.ns?.name ?? account?.title ?? "Unknown account",
  );
  let balance = $derived(
    account ? formatMicroAlgos(account.info?.amount) : "—",
  );
  let assetHoldings = $derived(accountAssets(account));
  let explorerUrl = $derived(
    account ? explorerAccountUrl(app.selectedNetwork, account.addr) : undefined,
  );
  let kindVariant = $derived(
    account ? accountKindVariant(account) : "secondary",
  );
  let refreshErrorMessage = $derived(
    accountQuery.error?.message ?? "Unable to refresh account data.",
  );

  function accountKindVariant(
  ...
  const rekeyedFromAccounts = $derived(
    app.accounts.filter(
      (a) => a !== account && a.info?.authAddr === account?.addr,
    ),
  );
...
{#snippet LoadingAccount()}
  <Card.Root>
    <Card.Header>
      ...
      <Card.Description
        >Refreshing account balances, assets, and name metadata.</Card.Description
      >
    </Card.Header>
...
{#snippet AccountNotFound()}
  <Alert.Root variant="destructive">
    ...
    <Alert.Description
      >No account with address {addr} was found in the wallet.</Alert.Description
    >
  </Alert.Root>
...
{#snippet AccountHeader()}
  ...
  >
    <div class="flex items-center gap-3">
      <Button variant="ghost" size="sm" onclick={() => push("/accounts")}
        >← Back</Button
      >
      <h2 class="text-2xl font-mono tracking-tight">Account Details</h2>
    </div>
...
{#snippet SummaryCards()}
  <div class="grid gap-3 sm:grid-cols-3">
    ...
    {@render SummaryCard(
      "Signability",
      account?.canSign ? "Can sign" : "Watch only",
    )}
  </div>
{/snippet}
    ...
    <Card.Header>
      <div class="flex items-start justify-between gap-3">
        ...
        <Badge variant={kindVariant}
          >{account ? accountKind(account) : "Unknown"}</Badge
        >
      </div>
    </Card.Header>
...
{#snippet OptionalAccountRows()}
  {#if account?.seedId !== undefined}
    <span class="text-muted-foreground">Seed ID</span><span
      >{account.seedId}</span
    >
  {/if}
  ...
  {/if}
  {#if account?.xpub}
    <span class="text-muted-foreground">Extended Public Key</span><span
      class="break-all font-mono text-xs">{account.xpub.slice(0, 32)}...</span
    >
  {/if}
  {#if account?.idxs && account.idxs.length > 0}
    <span class="text-muted-foreground">Address Indexes</span><span
      >{account.idxs.join(", ")}</span
    >
  {/if}
  {#if account?.appId}
    <span class="text-muted-foreground">App ID</span><span
      >{account.appId.toString()}</span
    >
  {/if}
  {#if account?.falcon}
    <span class="text-muted-foreground">Falcon counter</span><span
      >{account.falcon.counter}</span
    >
  {/if}
  {#if account?.network}
    <span class="text-muted-foreground">Restricted to</span><span
      >{account.network}</span
    >
  {/if}
  {#if account?.name}
    <span class="text-muted-foreground">Custom name</span><span
      >{account.name}</span
    >
  {/if}
{/snippet}
...
{#snippet RekeyStatus()}
  {#if account?.info?.authAddr}
    ...
    <div class="grid gap-2">
      ...
      <p class="text-sm">
        This account is rekeyed to <code
          class="rounded bg-muted px-1 py-0.5 text-xs break-all"
          >{account.info.authAddr}</code
        >.
      </p>
      {#if rekeyedToAccount}
  ...
  {#if rekeyedFromAccounts.length > 0}
    ...
    <div class="grid gap-2">
      ...
      {#each rekeyedFromAccounts as rekeyed (rekeyed.addr)}
        <Button
          variant="link"
          class="h-auto w-fit p-0 text-sm"
          onclick={() => push(`/account-detail/${rekeyed.addr}`)}
        >
    ...
    <div class="grid gap-2">
      ...
      <div class="grid grid-cols-[auto_1fr] gap-x-4 gap-y-1 text-sm">
        <span class="text-muted-foreground">Name</span><span
          >{account.ns.name}</span
        >
        {#if account.ns.appID}
          <span class="text-muted-foreground">App ID</span><span
            >{account.ns.appID}</span
          >
        {/if}
      </div>
    </div>
  ...
  {#if account?.appId}
    <Card.Root>
      <Card.Header>
        ...
        <Card.Description
          >Smart-contract-backed multisig account summary.</Card.Description
        >
      </Card.Header>
      <Card.Content class="grid gap-2 text-sm">
        <div class="grid grid-cols-[auto_1fr] gap-x-4 gap-y-1">
          <span class="text-muted-foreground">App ID</span><span
            >{account.appId.toString()}</span
          >
          <span class="text-muted-foreground">Address</span><span
            class="break-all">{account.addr}</span
          >
          <span class="text-muted-foreground">Signability</span><span
            >{account.canSign ? "Local signer available" : "Watch only"}</span
          >
          {#if account.network}
            <span class="text-muted-foreground">Network</span><span
              >{account.network}</span
            >
          {/if}
        </div>
        <Button
          ...
          size="sm"
          class="w-fit"
          onclick={() => push(`/multisig/${account.appId!.toString()}`)}
        >
          Manage Multisig
        </Button>
      </Card.Content>
...
{#snippet AssetsCard()}
  <Card.Root>
    <Card.Header>
      ...
      <Card.Description
        >ASA holdings with cached metadata and resolved media URLs.</Card.Description
      >
    </Card.Header>
    <Card.Content class="grid gap-3">
      {#if assetHoldings.length === 0}
        <p class="text-sm text-muted-foreground">
          No ASA holdings found for this account.
        </p>
      {:else}
