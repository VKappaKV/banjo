<script lang="ts">
  ...
  let lastUpdated = $derived(
    accountsQuery.dataUpdatedAt > 0
      ? new Date(accountsQuery.dataUpdatedAt).toLocaleTimeString()
      : "Never",
  );
  let explorerUrl = $derived.by(
    () => (address: string) => explorerAccountUrl(app.selectedNetwork, address),
  );
  let hotWalletVariant = $derived<BadgeVariant>(
    app.state.hotWalletEnabled ? "default" : "secondary",
  );
...
<div class="grid gap-4">
  ...
  >
    <div>
      <h2 class="text-2xl font-mono tracking-tight">Accounts</h2>
      <p class="text-muted-foreground text-sm">
        Loaded from Banjo core state for {app.state.networkName}.
      </p>
    </div>
    <div class="flex flex-wrap gap-2">
      ...
      <Button
        ...
        disabled={accountsQuery.isFetching || !app.initialized}
      >
    ...
    <Card.Root>
      ...
      <Card.Content>
        <p class="text-sm text-muted-foreground">
          {summary.totalAssets} total holdings
        </p>
      </Card.Content>
    </Card.Root>
    <Card.Root>
  ...
  {#if app.initializing}
    <Card.Root>
      <Card.Header>
        ...
        <Card.Description
          >Opening IndexedDB and checking browser capabilities.</Card.Description
        >
      </Card.Header>
    </Card.Root>
      ...
      {#each [1, 2, 3] as item (item)}
        <Card.Root>
          <Card.Header>
            ...
            <div
              class="bg-muted mt-2 h-4 w-full max-w-lg animate-pulse rounded"
            ></div>
          </Card.Header>
          <Card.Content>
      ...
      <Card.Content>
        <div class="flex flex-wrap gap-2">
          ...
          <Badge variant={hotWalletVariant}>
            Hot wallet {app.state.hotWalletEnabled
              ? "available"
              : "unavailable"}
          </Badge>
        </div>
  ...
  {:else}
    <div class="grid gap-3">
      {#each accounts as account (account.addr)}
        <AccountCard
          {account}
          explorerUrl={explorerUrl(account.addr)}
