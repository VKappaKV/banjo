<script lang="ts">
  ...
  let targetNetwork = $derived.by(() => {
    if (request.request?.action !== "connect" || !request.request.genesisID)
      return undefined;
    try {
      return resolveConnectNetwork({
        genesisID: request.request.genesisID,
  ...
  let filterOptions = $derived<FilterOption[]>(
    (
      ...
      ] satisfies Array<[AccountFilter, string]>
    ).map(([value, label]) => ({
      value,
      label,
  ...
  let filteredAccounts = $derived.by(() => {
    if (filter === "signable")
      return app.accounts.filter((account) => account.canSign);
  ...
  async function approve() {
    ...
    try {
      ...
      if (!request.request.genesisID)
        throw new Error("Connect request is missing a genesis ID.");
      const network = resolveConnectNetwork({
        genesisID: request.request.genesisID,
        networks: app.allNetworks,
      });
...
<div class="grid gap-6">
  <div>
    <h2 class="text-2xl font-mono tracking-tight">Connect dApp</h2>
    <p class="text-muted-foreground text-sm">
      Review the dApp request and choose which accounts to expose.
    </p>
  </div>
  ...
  {:else if request.error}
    <Alert.Root variant="destructive"
      ><Alert.Description>{request.error}</Alert.Description></Alert.Root
    >
  {:else if request.done}
    <Card.Root
      ><Card.Header
        ><Card.Title>Connect response sent</Card.Title><Card.Description
  ...
  {:else if request.request?.action !== "connect"}
    <Alert.Root variant="destructive"
      ><Alert.Description>No connect request is pending.</Alert.Description
      ></Alert.Root
    >
  {:else}
    <Card.Root>
      <Card.Header>
        ...
        <Card.Description
          >Requested network: {request.request.genesisID}</Card.Description
        >
      </Card.Header>
      <Card.Content class="grid gap-4">
        <div class="flex flex-wrap gap-2">
          <Badge variant={targetNetwork ? "default" : "destructive"}
            >{targetNetwork?.name ?? "Unknown network"}</Badge
          >
          {#if app.state.snoop}<Badge variant="secondary"
              >Snoop mode enabled</Badge
            >{/if}
          {#if app.state.debug}<Badge variant="outline"
              >Debug response enabled</Badge
            >{/if}
        </div>
        <div class="flex flex-wrap gap-2">
        ...
        />
        {#if connectError}<Alert.Root variant="destructive"
            ><Alert.Description>{connectError}</Alert.Description></Alert.Root
          >{/if}
      </Card.Content>
