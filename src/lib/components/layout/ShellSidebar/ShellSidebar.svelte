<script lang="ts">
  ...
  type SidebarItem = (typeof sidebarViewDefinitions)[number] & {
...
<aside
  class="border-border bg-sidebar text-sidebar-foreground border-b p-4 lg:border-r lg:border-b-0"
>
  <div class="flex items-center justify-between gap-3 lg:block">
    <div>
      <p
        class="font-mono text-xs uppercase tracking-[0.35em] text-muted-foreground"
      >
        Banjo
      </p>
      <h1 class="mt-1 text-2xl font-mono tracking-tight">Wallet Shell</h1>
    </div>
    {#if app.isLoading}
